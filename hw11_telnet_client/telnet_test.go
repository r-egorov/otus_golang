...
func TestTelnetClientFail(t *testing.T) {
	t.Run("unknown host", func(t *testing.T) {
		...
		client := NewTelnetClient("unknownhost", timeout, ioutil.NopCloser(in), out)
		require.Error(t, client.Connect())
	})
	t.Run("send and receive without connecting", func(t *testing.T) {
		l, err := net.Listen("tcp", "127.0.0.1:")
		require.NoError(t, err)
		defer func() { require.NoError(t, l.Close()) }()
